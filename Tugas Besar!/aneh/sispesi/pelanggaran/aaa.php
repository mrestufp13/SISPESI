...
if(isset($_GET['cari'])){
	...
	$tampil = "SELECT * FROM pelanggaran WHERE jns_pel LIKE '%$q%' OR level LIKE '%$q%'  ORDER BY jns_pel LIMIT $posisi, $batas";
}else{
...
if($jmlhasil < 1){
	...
	while($data=mysqli_fetch_array($hasil)){
		...
		echo "<td><a href='hapussiswa.php?no=$data[kd_pel]'>Hapus</a> | <a href='editsiswa.php?no=$data[kd_pel]'>Edit</a> </td>";
		echo "</tr>";
		$no++;
		}
}
